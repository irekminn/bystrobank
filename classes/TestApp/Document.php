...
class TestApp_Document extends Adaptor_XMLBase implements Adaptor_Array {
		...
		while($xr->read()){
			if($xr->nodeType==XMLReader::ELEMENT) {
				...
				switch($xr->localName){
					...
					case "deleted": $this->deleted=$xsinil?NULL:new Basictypes_Boolean($xr->readString(),Adaptor_DataType::SQL); break;
	...
	public function fromArray($row,$mode=Adaptor_DataType::XSD){
		...
		if(isset($row["deleted"])) $this->deleted=new Basictypes_Boolean($row["deleted"],$mode);
